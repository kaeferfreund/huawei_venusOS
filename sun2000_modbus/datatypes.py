...
def decode(value, data_type):
    ...
    if data_type == DataType.UINT16_BE or data_type == DataType.UINT32_BE:
            return decode_uint_be(value)
    if data_type == DataType.INT16_BE or data_type == DataType.INT32_BE:
            return decode_int_be(value)
    if data_type == DataType.BITFIELD16 or data_type == DataType.BITFIELD32:
            return decode_bitfield(value)
    if data_type == DataType.MULTIDATA:
            return value
    else:
